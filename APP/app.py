...
def calc_custom_score(f):
        ...
        score = 10*(1.2*A - 0.9*B) + 30*(1.5*C - 1.3*D) + 0.65*(E + F + G) + 2*H + 15*(1.5*I + 0.85*K - 1.2*J - L) + 10*(1.5*M - 0.75*N)+ 10*Q*(1.25*O + 0.8*P)
    ...
    with st.expander("📋 Διαχείριση Παρολί & Προσθήκη νέου αγώνα", expanded=True):
        ...
        if selected_f1 == selected_f2:
            ...
            if st.button("🧾 Προσθήκη στο Παρολί", key="exp_add_to_paroli"):
                ...
                st.experimental_rerun()  # ανανέωση για να δεις τη νέα λίστα αμέσως
        ...
        if st.session_state.multi_fights:
            ...
            for idx, fight in enumerate(st.session_state.multi_fights):
                ...
                with col1:
                    st.markdown(f"**{idx+1}. {fight['f1']} vs {fight['f2']} → 🏆 {fight['winner']} ({fight['prob']}%)**")
                with col2:
