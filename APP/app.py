...
#-------Παρολι--------
   ...
   # ------- VALUE BET --------
    ...
    def calc_custom_score(f):
        age = f["Age"]
        if 24 <= age <= 27 or 33 <= age <= 36:
            E = 3.5
        elif 28 <= age <= 32:
            E = 4
        elif age in [21, 22, 23, 37]:
            E = 2
        elif age in [18, 19, 20, 38]:
            E = 1
        elif 39 <= age <= 40:
            E = -1
        elif age >= 41:
        ...
        F = (f["Height"] - 155) * 0.12 + 1 if f["Height"] >= 155 else 0
        G = (f["Reach"] - 155) * 0.14 + 1 if f["Reach"] >= 155 else 0
        A = f["Sig Strikes Landed"]
        B = f["Sig Strikes Absorbed"]
        C = f["Control %"] / 100
        D = f["Controlled %"] / 100
